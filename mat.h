...
public:
	...
	SMat<T>* prod(SMat<T>* mat2, int k = -1, float th = -0.1, string agg_type = "sum", int num_thread = 1, bool pp = false)
	{
		...
		if(pp) { LOGN("prod called with k : " << k << " th : " << th << " num thread : " << num_thread << " lmat : (" << nr << ", " << nc << ") rmat : (" << mat2->nr << ", " << mat2->nc << ")"); }
